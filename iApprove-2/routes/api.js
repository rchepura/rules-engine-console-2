...
router.delete('/*', function (req, res) {
    ...
    if ( req.query.userID || ( (req.query.ideaId || req.query.topicId) && req.query.commentId ) ) {
        sendReq(req, res);
        return;
    }
...
function sendReq(req, res, body, file) {
    ...
    if ( 'DELETE' == req.method ) {
        if ( req.query.userID ) {
            bodyData = require('querystring').stringify({
                userId:req.query.userID
            });
        } else if ( (req.query.ideaId || req.query.topicId) && req.query.commentId ) {
            var delData = {
                ideaId: req.query.ideaId,
                commentId: req.query.commentId
            }
            if ( req.query.ideaId ) {
                delData.ideaId = req.query.ideaId;
            } else {
                delData.topicId = req.query.topicId;
            }
            bodyData = require('querystring').stringify(delData);
        }
